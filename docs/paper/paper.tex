% Preprint draft — builds with pdflatex (TinyTeX); figures from make_figures.py.
...
% Small captions with bold labels (kernel-level; avoids the caption package).
\makeatletter
...
of ten, and its failures are informative: the missing events concern long
...
absorption kills the fat tails, leverage asymmetry and volatility clustering
...
A quarter century ago the stylized-facts program gave empirical finance a
shared target: a short list of statistical regularities --- heavy tails,
volatility clustering, the leverage effect, near-absence of linear return
autocorrelation --- that any candidate mechanism for price formation should
reproduce \citep{cont2001}. Agent-based modeling took up the challenge early.
Heterogeneous-agent markets with fundamentalists and chartists were shown to
generate fat tails and clustered volatility from agent interaction rather than
from exogenous news \citep{lux1999}, and a substantial literature has since
mapped which ingredients produce which facts \citep{lebaron2006}.

Two gaps in that literature motivate this paper. First, scoring is usually
...
rather than a ledger of what a given mechanism class can and cannot buy.
Second, negative results are structurally underproduced: when an added
ingredient fails to improve a model, the failure is rarely reported, almost
never pre-registered, and so carries no evidential weight.

We address both gaps with a fixed measurement instrument and a pre-registered
refutation chain.
...
The paper's headline mechanism result is easy to state. Adding one layer of
anticipation to the minimal market leaves its score unchanged and moves the
sign leak one derivative earlier; iterating anticipation five deep contracts
the model-space forecastable flow by a factor of four while the leaked sign
information in the realized prices \emph{rises} monotonically
(0.0184 $\to$ 0.0200 $\to$ 0.0257 bits); and a maker that removes the
...
Table~\ref{tab:events} lists them. The events are close-only versions of
regularities measured and gate-validated in a prior program on $\sim$48 liquid
US equities and ETFs (2010--2026), so the thresholds are empirical ranges, not
conventions.
...
E8 & arrow of time in the coupling & entropy production significant in $r$, not in vol-deformed $r$ \\
E9 & no sign information & direction bits $\le$ shuffle null (95\%) \\
E10 & tail asymmetry & gain/loss tail asymmetry at 2.5$\sigma$ \\
\bottomrule
\end{tabular}
\caption{The ten-event battery (final calibrated form). E1, E4, E7 and E10
...
memory. E9 measures the mutual information between public features and the
next-day return sign with a KSG estimator against a shuffle null
\citep{kraskov2004}.}
% src: docs/design/DESIGN8.md events + calibration-phase amendments
\label{tab:events}
\end{table}
...
between 3 and 10 is what a mechanism must buy.
% src: research/decathlon.json spy.score, configs.G

\subsection{The minimal market}

The base market is deliberately small: aggregated flows, roughly eight
...
$D^{\mathrm{vol}} = k_V\,(L_t - L_{t-1})$ for volatility-targeters running the
...
even efficiency. The volatility-targeters are the star ingredient:
...
(long-memory clock), E7 (clock up-jumps, marginally), E8 (the arrow in the
coupling), and E9 (information-free signs: 0.0184 significant direction bits).
The E9 leak has a known source: the vol-targeters' de-/re-leveraging drift is
serially correlated and forecastable from the tape, and a one-lag market maker
cannot absorb a multi-day drift. The natural hypothesis --- the one this
...
inventory change plus execution noise. The unwind is automatic: as vol reverts
...
$(k_A, \mathrm{cap}_A, s_A)$, frozen before any battery run.
...
identical and the direction bits go \emph{up}.}
...
went up, not away ($0.0184 \to 0.0200$), and the long-memory events stayed
flat. The interaction hypothesis held: without vol-targeters the anticipator
...
\bottomrule
\end{tabular}
\caption{Experiment II: the $K$-ladder. The sign leak grows monotonically in
$K$ while the model-space forecastable flow contracts toward zero; the frozen
$K{=}5$ stack is a two-event regression (it breaks efficiency and kills the
leverage effect).}
...
market at the same $K$ (open circles: the eight evaluation seeds), rising
monotonically $0.0184 \to 0.0200 \to 0.0257$. The operator removes forecastable
...
market-space sign leak rises $0.0184 \to 0.0200 \to 0.0257$ bits. The frozen
...
flow down, market-space sign leakage up: this is the cleanest statement we
...
diagnosis: price-\emph{setting} rationality. A market maker who does not trade
but quotes --- pre-adjusting the price by the expected impact of the
...
\end{equation}
%
with skew strength $\lambda_Q \in \{1.0, 0.5\}$ fixed in advance
...
bits (bottom right; open circles are seeds) do not close: flat at half skew,
higher at full skew.}
...
major coins, 2017--2026. The ordering $-0.041 < +0.005 < +0.031$ is monotone
in point estimates; the equity edge is statistically certified, the FX--crypto
edge is inside one-sided noise.}
% src: research/fx.json leverage_contrast (equity_mean, equity_spread, fx_leverage, fx_sd, z values, n_pairs_positive); research/crypto.json leverage_contrast, per_coin_leverage; docs/FINDINGS.md KRONOS-FX table (z vs zero: ~-5 equities, 1.96 crypto)
\label{tab:triangle}
\end{table}

Table~\ref{tab:triangle} and Figure~\ref{fig:triangle} give the measurement,
using the identical estimator battery at all three vertices (realized variance
from daily OHLC ranges via Garman--Klass \citep{garman1980}). Equity markets
...
$-0.047$). FX --- an institutional dealer market where a currency is not a
levered claim on anything and no retail-momentum dynamic dominates --- is
...
zero at only $z \approx 1.96$ --- so ``crypto genuinely positive above an FX
...
continuous-integration suite runs 37 numbered gates on every commit.
% src: docs/METHODS.md sections 0 and 8

\subsection{Pre-registration, budgets, and tie-breaks}

Each experiment fixed, before any evaluation run: hypotheses (with one starred
primary), the full parameter set, the evaluation protocol (8 seeds, IDs
100--107, $T = 6000$, majority vote per event, identical across all three
experiments so ablations are comparable), a kill criterion, and a bounded
tuning budget --- one grid pass on disjoint tuning seeds (900--903), permitted
...
% src: docs/design/DESIGN20.md, DESIGN22.md (gates X32a, X34a); CLAUDE.md (arch-dependent-pin lesson)
...
% src: docs/METHODS.md sections 5 and 8; docs/FINDINGS.md (PBO 0.45 standing rule)
...
battery's E8 event.
% src: docs/FINDINGS.md KRONOS-REFLEX (0.68 -> 0.25)
Rough volatility \citep{gatheral2018} motivates the battery's long-memory
clock event E4: the measurement program replicated $H \approx 0.10$ on 16
...
flow-generated.
% src: docs/FINDINGS.md KRONOS-X Q3b (H ~= 0.10); docs/design/DESIGN8.md amendment 1 (GJR fails E4)
