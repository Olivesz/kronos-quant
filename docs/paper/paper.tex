...
This appendix states each event's statistic and pass region exactly as
...
its calibration-phase amendments). Notation: $r_t$ are daily returns;
$\mathrm{RV}_t$ is the 5-day rolling mean of $r^2$ (the close-only volatility
clock), $\sigma_t = \sqrt{\mathrm{RV}_t}$, and $z_t = r_t/\sigma_t$ the
...
but E9's features are discrete by construction.}
